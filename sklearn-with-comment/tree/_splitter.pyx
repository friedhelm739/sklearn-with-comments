    ...
    cdef int init(self,
        ...
        for i in range(n_samples):
            ...
            if sample_weight == NULL or sample_weight[i] != 0.0:
                samples[j] = i   # sample 内为权重不为0的有效样本id
                j += 1

            if sample_weight != NULL:
                weighted_n_samples += sample_weight[i]  # weighted_n_samples 计算所有样本权重的和
            else:
            ...
            if f_j < n_known_constants:
                ...
                if self.presort == 1:
                    ...
                    for i in range(self.n_total_samples): 
                        j = X_idx_sorted[i + feature_idx_offset]       # 内存操作，读取每一样本并按当前特征排序
                        if sample_mask[j] == 1:
                            samples[p] = j
